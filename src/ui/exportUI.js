/**
 * EXPORT UI — Save/Load/Export/Example
 */
...
export function exportReport() {
    ...
    const parts = document.getElementById('docParticipants').value || '—';
    const rev = document.getElementById('docReviewer').value || '—';
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Threat Model — ${name}</title>
...
<body><h1>🛡 Threat Model Report</h1><p style="color:#666;margin-top:-4px">Generated by ThreatCanvas · OWASP Threat Modeling Process</p>
${execHtml}
...
<h2>2. Architecture — DFD Components</h2>
...
export function saveProject() {
    ...
    const btn = document.querySelector('.btn-save'); if (btn) { const orig = btn.textContent; btn.textContent = '✓ Saved!'; btn.style.color = '#00ff99'; setTimeout(() => { btn.textContent = orig; btn.style.color = ''; }, 1800); }
}

function _bTR(i, c) { return `<tr><td>TL-${i}</td><td contenteditable="true" style="outline:none;color:var(--text)">${c[1] || ''}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${c[2] || ''}</td><td><button class="del-btn" onclick="this.closest('tr').remove()">✕</button></td></tr>`; }
function _bER(i, c) { return `<tr><td>EP-${i}</td><td contenteditable="true" style="outline:none;color:var(--text)">${c[1] || ''}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${c[2] || ''}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${c[3] || ''}</td><td><button class="del-btn" onclick="this.closest('tr').remove()">✕</button></td></tr>`; }
...
function _bAR(i, c) { return `<tr><td>A-${i}</td><td contenteditable="true" style="outline:none;color:var(--text)">${c[1] || ''}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${c[2] || ''}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${c[3] || ''}</td><td><button class="del-btn" onclick="this.closest('tr').remove()">✕</button></td></tr>`; }
function _bDR(i, c) { return `<tr><td>DEP-${i}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${c[1] || ''}</td><td><button class="del-btn" onclick="this.closest('tr').remove()">✕</button></td></tr>`; }
...
export function loadExample() {
    ...
    [['User Login Credentials', 'Passwords in DB', 'TL-2, TL-4, TL-5'], ['Personal Data (PII)', 'Student info', 'TL-3, TL-4'], ['Library Availability', '24/7 uptime', 'TL-4, TL-5'], ['Audit Log Data', 'User ops trail', 'TL-3']].forEach(([n, d, t]) => { ROW_COUNTS.assets++; document.getElementById('assetsTbody').insertAdjacentHTML('beforeend', `<tr><td>A-${ROW_COUNTS.assets}</td><td contenteditable="true" style="outline:none;color:var(--text)">${n}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${d}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${t}</td><td><button class="del-btn" onclick="this.closest('tr').remove()">✕</button></td></tr>`); });
    // External deps
    document.getElementById('depsTbody').innerHTML = ''; ROW_COUNTS.deps = 0;
    [['Apache on hardened Linux'], ['MySQL on hardened Linux — private network'], ['Web-to-DB over private network'], ['Perimeter firewall — TLS only']].forEach(([d]) => { ROW_COUNTS.deps++; document.getElementById('depsTbody').insertAdjacentHTML('beforeend', `<tr><td>DEP-${ROW_COUNTS.deps}</td><td contenteditable="true" style="outline:none;color:var(--text2)">${d}</td><td><button class="del-btn" onclick="this.closest('tr').remove()">✕</button></td></tr>`); });
    // Build DFD
