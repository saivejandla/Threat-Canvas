...
function _modalHTML(m, appName, appVer, docOwner, docDate, mitigPct) {
    ...
    const dc = m.detectorNodes.length ? m.detectorNodes.map(n => `<span class="detect-node-chip" style="background:rgba(52,211,153,.12);color:var(--low);border:1px solid rgba(52,211,153,.3)">${escHtml(n.label)}</span>`).join('') : `<span class="detect-node-chip" style="background:rgba(239,68,68,.12);color:var(--crit);border:1px solid rgba(239,68,68,.3)">None detected</span>`;
    return `<div class="exec-header"><div><div class="exec-title">📋 Executive Security Summary</div><div class="exec-subtitle">${escHtml(appName)} v${escHtml(appVer)} · ${escHtml(docOwner)} · ${escHtml(docDate)}</div></div><button class="exec-close" id="execCloseBtn" title="Close">✕</button></div>
